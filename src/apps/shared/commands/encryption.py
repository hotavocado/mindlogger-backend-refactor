...
async def reencrypt(
    decrypt_secret_key: Optional[str] = typer.Option(
        ...
        "--decrypt-secret-key",
        "-d",
        help="Hex of secret key to decrypt data, "
        "if not key specified, default settings key will be used.",
    ),
    encrypt_secret_key: Optional[str] = typer.Option(
        None,
        "--encrypt-secret-key",
        "-e",
        help="Hex of secret key to encrypt data, "
        "if not key specified, default settings key will be used.",
    ),
    tables: Optional[list[str]] = typer.Argument(
        None, help="table where to reencrypt data"
    ),
) -> None:
    session_maker = session_manager.get_session()
    decrypt_key = (
        bytes.fromhex(decrypt_secret_key) if decrypt_secret_key else get_key()
    )
    decryptor = StringEncryptedType(Unicode, decrypt_key)
    encrypt_key = (
        bytes.fromhex(encrypt_secret_key) if encrypt_secret_key else get_key()
    )
            ...
            async with atomic(session):
                for table_name in tables:
                    ...
                    result = await session.execute(
                        f"select id, {cols} from {table_name}"
                    )
                    for row in result:
                        id_ = row.id
                        to_update = {}
                        for col in columns:
                            ...
                            if value:
                                ...
                                encrypted_value = encryptor.process_bind_param(
                                    decrypted_value, session.bind.dialect
                                )
